...
class MagicChess : AppCompatActivity() {
    ...
    private fun setupInitialPosition(color: String) {
        ...
        // Выбираем стартовую позицию в зависимости от цвета
        ...
        for (row in 0 until boardSize) {
            for (col in 0 until boardSize) {
                ...
                finalPosition[row][col]?.let { piece ->
                    val resource = pieces[piece] ?: R.drawable.empty
    ...
    private fun switchPlayer() {
        ...
        // Проверяем состояние игры после смены игрока
    ...
    private fun isValidPawnMove(piece: String?, fromRow: Int, fromCol: Int, toRow: Int, toCol: Int): Boolean {
        ...
        val adjustedDirection = when {
            currentPlayerColor == "white" && piece == "white_pawn" -> -1
            currentPlayerColor == "black" && piece == "black_pawn" -> -1
            currentPlayerColor == "white" && piece == "black_pawn" -> 1
            currentPlayerColor == "black" && piece == "white_pawn" -> 1
            else -> 0
        }
    ...
    private fun isValidRookMove(fromRow: Int, fromCol: Int, toRow: Int, toCol: Int): Boolean {
        ...
        if (rowDiff > 3 || colDiff > 3) return false
    ...
    private fun isValidKnightMove(fromRow: Int, fromCol: Int, toRow: Int, toCol: Int): Boolean {
        ...
        val isValidMove = (Math.abs(rowDiff) == 3 && Math.abs(colDiff) == 1) ||
                (Math.abs(rowDiff) == 1 && Math.abs(colDiff) == 3)
    ...
    private fun setupTimer(timeLimit: String) {
        ...
        whiteTimer = object : CountDownTimer(whiteTimeMillis, 1000) {
            ...
            override fun onFinish() {
                if (timeLimit != "without") {
                    Toast.makeText(this@MagicChess, "Время вышло! Побеждают Черные", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }
        }
        blackTimer = object : CountDownTimer(blackTimeMillis, 1000) {
            ...
            override fun onFinish() {
                if (timeLimit != "without") {
                    Toast.makeText(this@MagicChess, "Время вышло! Побеждают Белые", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }
        }
